...
const WorkflowManagementPage = () => {
    ...
    const getStatusBadge = (status: string) => {
        switch (status.toLowerCase()) {
            case 'active':
            case 'success': 
                return <span className="px-3 py-1 bg-[#00BA4A]/10 text-[#00BA4A] text-[10px] font-black uppercase rounded-full border border-[#00BA4A]/20">Đã kích hoạt</span>;
            case 'pending':
            ...
            default:
                return <span className="px-3 py-1 bg-gray-500/10 text-gray-500 text-[10px] font-black uppercase rounded-full border border-gray-500/10">{status}</span>;
        }
    };
            ...
            <div className="bg-gray-50 dark:bg-[#060a09] min-h-screen pb-24 overflow-x-hidden">
                ...
                <div className="max-w-7xl mx-auto px-4 py-3">
                    <nav className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[2px] transition-all duration-300">
                        ...
                        <Link to="/landing-workflows" className="text-gray-400 hover:text-[#00BA4A] transition-colors">Giải pháp</Link>
                        <FeatherIcon icon="chevron-right" size={10} className="text-gray-300" />
                        <span className="text-gray-900 dark:text-white">Kho workflow của tôi</span>
                    </nav>
                </div>
                    ...
                    <div className="max-w-7xl mx-auto px-4 py-12 relative z-10">
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
                            <div className="space-y-4">
                                <div className="space-y-2">
                                    <h1 className="text-4xl md:text-6xl font-black !text-white uppercase tracking-tighter leading-none mb-1">
                                        QUẢN LÝ <span className="text-[#00BA4A]">WORKFLOWS</span>
                                    </h1>
                            ...
                            <Link 
                                ...
                                className="px-8 h-14 rounded-xl font-black uppercase tracking-[2px] transition-all shadow-xl shadow-[#00BA4A]/20 text-[11px] flex items-center gap-3 bg-[#00BA4A] text-white hover:brightness-105"
                            >
                ...
                <div className="max-w-7xl mx-auto px-4 py-10">
                    <div className="bg-white dark:bg-[#0d1412] rounded-xl border border-gray-100 dark:border-white/5 shadow-sm overflow-hidden">
                        {/* Tabs Navigation */}
                        <div className="flex items-center border-b border-gray-100 dark:border-white/5 px-6">
                            {tabs.map(tab => (
                                <button
                                    ...
                                    className={`flex items-center gap-2 py-6 px-6 text-[11px] font-black uppercase tracking-widest transition-all relative ${activeTab === tab.id ? 'text-[#00BA4A]' : 'text-gray-400 hover:text-gray-600 dark:hover:text-white'}`}
                                >
                        ...
                        <div className="p-0">
                            {loading ? (
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left">
                                        ...
                                        <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                                            {paginatedWorkflows.map((wf) => (
                                                <tr key={wf.id} className="hover:bg-gray-50/50 dark:hover:bg-white/[0.02] transition-colors group">
                                                    <td className="p-6">
                                                        <div className="flex items-center gap-4">
                                                            <div className="w-12 h-12 bg-gray-100 dark:bg-white/5 rounded-xl overflow-hidden flex items-center justify-center">
                                                                {wf.image ? (
                                                                    <img src={wf.image} alt={wf.name} className="w-full h-full object-cover" />
                                                                ) : (
                                                                    <FeatherIcon icon="cpu" size={20} className="text-gray-400 group-hover:text-[#00BA4A]" />
                                                                )}
                                                            </div>
                                                            <div>
                                                                <span className="text-[14px] font-black dark:text-white uppercase tracking-tight block leading-none mb-1 group-hover:text-[#00BA4A] transition-colors">{wf.name}</span>
                                                                <span className="text-[9px] font-black text-gray-400 uppercase tracking-[1px] bg-gray-100 dark:bg-white/5 px-1.5 py-0.5 rounded">ID: #{wf.wfId || wf.id}</span>
                                                            </div>
                                                        </div>
                                                    </td>
                                                    <td className="p-6 text-center">
                                                        <span className="text-[13px] font-black text-[#00BA4A] tracking-tight">{fmt(wf.price)}</span>
                                                    </td>
                                                    <td className="p-6 text-center">
                                                        {getStatusBadge(wf.status)}
                                                    </td>
                                                    <td className="p-6 text-center">
                                                        <span className="text-[12px] font-black text-gray-500 dark:text-gray-400 tracking-tight">{wf.date}</span>
                                                    </td>
                                                    <td className="p-6 text-right">
                                                        <div className="flex items-center justify-end gap-3">
                                                            <Link to={`/landing-workflows/${wf.wfId}`} className="h-10 px-4 flex items-center justify-center rounded-lg border border-gray-100 dark:border-white/5 hover:bg-gray-100 dark:hover:bg-white/5 transition-all text-[10px] font-black uppercase tracking-widest text-gray-500 hover:text-white">
                                                                Chi tiết
                                                            </Link>
                                                            <button 
                                                                className="h-10 px-6 rounded-lg bg-[#00BA4A] text-white text-[10px] font-black uppercase tracking-widest shadow-lg shadow-[#00BA4A]/20 hover:brightness-110 active:scale-95 transition-all flex items-center gap-2"
                                                            >
                                    ...
                                    {totalPages > 1 && (
                                        <div className="flex flex-col md:flex-row items-center justify-between p-6 border-t border-gray-100 dark:border-white/5 bg-gray-50/10 gap-4">
                                            ...
                                            <div className="flex items-center gap-2">
                                                <button 
                                                    ...
                                                    className="w-10 h-10 flex items-center justify-center rounded-lg border border-gray-100 dark:border-white/5 bg-white dark:bg-transparent disabled:opacity-30 transition-all"
                                                >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-10">
                        <div className="bg-white dark:bg-[#0d1412] p-6 rounded-xl border border-gray-100 dark:border-white/5 flex items-center gap-5">
                        ...
                        <div className="bg-white dark:bg-[#0d1412] p-6 rounded-xl border border-gray-100 dark:border-white/5 flex items-center gap-5">
                            <div className="w-12 h-12 rounded-xl bg-[#00BA4A]/10 text-[#00BA4A] flex items-center justify-center">
