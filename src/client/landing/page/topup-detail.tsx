...
const LandingTopupDetailPage = () => {
  ...
  if (loading) return <HostingLayout><div className="min-h-screen flex items-center justify-center"><div className="w-12 h-12 border-4 border-[#FBBF24] border-t-transparent rounded-full animate-spin"></div></div></HostingLayout>;
    ...
    <HostingLayout>
      <div className="hidden md:block container   md:px-6 pt-1 pb-2 max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="flex items-center gap-3 text-[11px] font-black uppercase tracking-[2px] ">
          ...
          <Link to="/landing-recharge" className="text-gray-400 hover:text-[#FBBF24] transition-colors">Nạp tiền</Link>
          <FeatherIcon icon="chevron-right" size={10} className="text-gray-300" />
          <span className="text-white">Chi tiết nạp #{topup.code}</span>
        </div>
      </div>

      <div className="bg-[#060a09] min-h-screen pb-32 transition-colors duration-500">
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-8">
              <div className="space-y-3">
                <h1 className="text-4xl md:text-6xl font-black !text-white uppercase tracking-tighter leading-none">
                  CHI TIẾT <span className="text-[#FBBF24]">HÓA ĐƠN</span>
                </h1>
              ...
              <div className="flex items-center gap-3">
                <Link to="/landing-recharge?pay=recharge-bank" className="px-6 h-12 rounded-xl font-black uppercase tracking-[2px] transition-all text-[10px] flex items-center gap-2 bg-[#FBBF24] text-white shadow-lg shadow-[#FBBF24]/20">
                  <FeatherIcon icon="home" size={14} /> BANK TRANSFER
                </Link>
                <Link to="/landing-recharge?pay=recharge-crypto" className="px-6 h-12 rounded-xl font-black uppercase tracking-[2px] transition-all text-[10px] flex items-center gap-2 bg-[#0d1412]/5 text-white/40 hover:bg-[#0d1412]/10 border border-white/[0.03]">
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-in slide-in-from-bottom-6 duration-500 mb-8">
            ...
            <div className="flex flex-col gap-6">
              <div className="bg-[#0d1412] rounded-[10px] border border-white/[0.03] shadow-sm overflow-hidden flex flex-col h-full">
                ...
                <div className="p-6 md:p-8 flex-1 space-y-5">
                  ...
                  <div className="flex items-center justify-between border-b border-white/[0.03] pb-4">
                    ...
                    <span className="px-5 py-2 bg-[#FBBF24] text-force-white text-xl font-black rounded-[8px] tracking-tight">{(topup.amount || 0).toLocaleString('vi-VN')}₫</span>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-b border-white/[0.03] pb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="px-4 py-2 bg-[#0d1412]/5 border border-white/10 rounded-[8px] text-[15px] font-black dark:text-white family-mono">{topup.accountNumber}</span>
                      <button onClick={() => handleCopy(topup.accountNumber!)} className="p-2.5 bg-[#FBBF24]/5 hover:bg-[#FBBF24]/10 text-[#FBBF24] rounded-lg transition-all group" title="Copy">
                  ...
                  <div className="flex items-center justify-between border-b border-white/[0.03] pb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="px-4 py-2 bg-[#0d1412]/5 border border-white/10 rounded-[8px] text-[15px] font-black dark:text-white uppercase">{topup.accountOwner}</span>
                      <button onClick={() => handleCopy(topup.accountOwner!)} className="p-2.5 bg-[#FBBF24]/5 hover:bg-[#FBBF24]/10 text-[#FBBF24] rounded-lg transition-all group" title="Copy">
                        <FeatherIcon icon="copy" size={16} className="group-hover:scale-110" />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-col gap-3 pt-4">
                    ...
                    <div className="flex items-center justify-between bg-[#FBBF24]/10 border-2 border-dashed border-[#FBBF24]/30 rounded-[10px] p-5">
                      <span className="text-2xl font-black text-[#FBBF24] tracking-widest">{topup.code}</span>
                      <button onClick={() => handleCopy(topup.code)} className="flex items-center gap-2 px-4 py-2 bg-[#FBBF24] text-force-white rounded-lg font-black text-[10px] uppercase tracking-widest hover:brightness-110 shadow-lg shadow-[#FBBF24]/20 transition-all">
              ...
              {/* Create New Prompt */}
              <Link to="/landing-recharge" className="w-full h-14 bg-[#0d1412] border-2 border-dashed border-white/10 rounded-[10px] flex items-center justify-center gap-2 text-[11px] font-black uppercase tracking-widest text-gray-400 hover:border-[#FBBF24] hover:text-[#FBBF24] transition-all">
                <FeatherIcon icon="plus-circle" size={16} /> Tạo hóa đơn nạp tiền mới
              </Link>
            </div>

            {/* QR block (RIGHT) */}
            <div className="bg-[#0d1412] rounded-[10px] border border-white/[0.03] shadow-sm overflow-hidden flex flex-col">
              <div className="bg-gradient-to-r from-[#FBBF24] to-[#FDE047] px-6 py-4 flex items-center gap-3 text-force-white">
              ...
              <div className="p-8 md:p-10 flex flex-col items-center justify-center space-y-8 h-full">
                <div className="p-6 bg-[#0d1412] rounded-2xl shadow-[0_20px_50px_rgba(0,0,0,0.1)] border-4 border-gray-50 group transition-transform hover:scale-105 duration-500">
                  <img src={qrCodeUrl} alt="VietQR" className="w-[180px] h-[180px] object-contain" />
                </div>

                <div className="w-full space-y-3">
                  <a href={qrCodeUrl} download={`3hst-qr-${topup.code}.png`} className="w-full h-14 bg-[#FBBF24] text-force-white rounded-[10px] font-black text-[11px] uppercase tracking-widest flex items-center justify-center gap-3 hover:brightness-110 shadow-xl shadow-[#FBBF24]/20 transition-all">
                  ...
                  <p className="text-[10px] font-bold text-gray-400 text-center uppercase tracking-widest leading-loose">
                    Quét bằng ứng dụng ngân hàng để thanh toán <br /> <span className="text-[#FBBF24]">An toàn & Chính xác nhất</span>
                  </p>
                </div>

                {/* Improved Timer UX */}
                <div className="w-full flex flex-col items-center gap-3 pt-6 border-t border-white/[0.03]">
          ...
          {/* Better Note Area */}
          <div className="bg-orange-500/5 border-l-4 border-orange-500 p-6 rounded-[10px] mb-12 shadow-sm animate-in fade-in duration-1000">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div className="flex gap-3">
                ...
                <p className="text-[11px] font-bold text-gray-400 leading-relaxed">CHUYỂN KHOẢN <span className="text-orange-600">ĐÚNG SỐ TIỀN</span> VÀ <span className="text-orange-600">NỘI DUNG</span> ĐỂ ĐƯỢC CỘNG TIỀN TỰ ĐỘNG.</p>
              </div>
              <div className="flex gap-3">
                ...
                <p className="text-[11px] font-bold text-gray-400 leading-relaxed">THỜI GIAN XỬ LÝ GIAO DỊCH TỪ <span className="text-orange-600">1 - 5 PHÚT</span> SAU KHI CHUYỂN KHOẢN THÀNH CÔNG.</p>
              </div>
              <div className="flex gap-3">
                ...
                <p className="text-[11px] font-bold text-gray-400 leading-relaxed">NẾU QUÁ 5 PHÚT CHƯA NHẬN ĐƯỢC TIỀN, HÃY LIÊN HỆ <span className="text-blue-600">TELEGRAM @NTTHANHZ</span>.</p>
              </div>
            </div>
          </div>

          {/* Bottom History Table */}
          <div className="bg-[#0d1412] rounded-[10px] border border-white/[0.03] shadow-sm overflow-hidden animate-in fade-in duration-700">
              ...
              <table className="w-full text-left text-[11px] font-bold">
                ...
                <tbody className="divide-y divide-gray-100 dark:divide-white/5">
                  {history.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage).map((t, i) => (
                    <tr key={i} className="hover:bg-white/5 dark:hover:bg-[#0d1412]/5 transition-colors">
                      <td className="px-6 py-4">
                        <Link to={`/landing-topup/${t.code}`} className="text-blue-600 font-black hover:text-[#FBBF24] transition-colors">
                          {t.code}
                        </Link>
                      </td>
            ...
            {/* Pagination Controls */}
            <div className="p-6 border-t border-white/[0.03] flex flex-col md:flex-row items-center justify-between gap-6">
              ...
              <div className="flex items-center gap-2">
                {Array.from({ length: Math.min(5, Math.ceil(history.length / itemsPerPage)) }).map((_, i) => (
                  <button
                    ...
                    className={`w-8 h-8 rounded-full flex items-center justify-center text-[10px] font-black transition-all ${currentPage === i + 1 ? 'bg-[#FBBF24] text-white shadow-lg' : 'bg-transparent text-gray-400 hover:bg-gray-100 dark:hover:bg-[#0d1412]/5'}`}
                  >
                    {i + 1}
                  </button>
                ))}
