...
const WorkflowDetailPage = () => {
    ...
    const handleBuy = async () => {
        ...
        const result = await Swal.fire({
            title: 'Xác nhận mua?',
            html: `Bạn sẽ mua giải pháp "<b>${workflow.name}</b>"<br/>Giá: <b class="text-[#00BA4A]">${fmt(workflow.price)}</b>`,
            icon: 'question',
        ...
        return (
            <HostingLayout>
                <div className="min-h-screen bg-[#F8FAFB] dark:bg-[#060a09] flex flex-col items-center justify-center gap-4">
                    ...
                    <Link to="/landing-workflows" className="px-6 py-2 bg-[#00BA4A] text-white rounded-lg text-xs font-black uppercase">Quay lại</Link>
                </div>
            </HostingLayout>
        );
    }
    ...
    return (
        <HostingLayout>
            <div className="min-h-screen bg-[#F8FAFB] dark:bg-[#060a09] pt-0 pb-12">
                ...
                {/* ── BREADCRUMBS ── */}
                <div className="w-full bg-white dark:bg-[#0d1513] border-b border-gray-100 dark:border-white/5 mb-0">
                    <div className="max-w-7xl mx-auto px-4 py-3">
                        <div className="flex items-center gap-3 text-[10px] font-black uppercase tracking-[2px] text-gray-400">
                            ...
                            <Link to="/landing-workflows" className="hover:text-[#00BA4A]">Workflow</Link>
                            <FeatherIcon icon="chevron-right" size={10} className="opacity-40" />
                            <span className="text-gray-900 dark:text-white">{w.name}</span>
                        </div>
                    </div>
                </div>

                {/* ── HERO SECTION ── */}
                <div className="bg-gradient-to-r from-[#00BA4A] to-[#032030] dark:from-[#0a1411] dark:to-[#080d0c] py-12 mb-8 border-b border-white/5 text-white">
                    <div className="container mx-auto px-4 md:px-6">
                        <div className="flex flex-col md:flex-row gap-10 items-start md:items-center">
                            <div className="relative group shrink-0">
                                <div className="absolute -inset-1 bg-gradient-to-r from-[#00BA4A] to-blue-500 rounded-2xl blur opacity-20 group-hover:opacity-40 transition duration-1000"></div>
                                <div className="relative w-full md:w-[280px] h-[190px] bg-gradient-to-br from-gray-100 to-gray-300 dark:from-gray-800 dark:to-gray-900 rounded-xl overflow-hidden shadow-2xl flex flex-col justify-between p-5 border border-white/10">
                                    <div className="flex justify-between items-start">
                                        <div className="w-10 h-10 rounded-lg bg-[#00BA4A]/10 flex items-center justify-center">
                                            <FeatherIcon icon="activity" size={24} className="text-[#00BA4A]" />
                                        </div>
                                        <span className="text-[9px] font-black bg-[#00BA4A] px-2 py-0.5 rounded text-white uppercase shadow-lg tracking-widest">Solution</span>
                                    </div>
                                    <div>
                                        <h3 className="text-gray-900 dark:text-white font-black text-lg mb-1 tracking-tight">{w.name}</h3>
                                        <div className="bg-black/5 dark:bg-white/10 px-2 py-0.5 rounded text-[10px] font-bold w-fit uppercase tracking-wider text-gray-700 dark:text-white">Workflow Engine</div>
                                    </div>
                                </div>
                            </div>
                ...
                <div className="container mx-auto px-4 md:px-6">
                    <div className="flex flex-col lg:flex-row gap-10 items-start">
                        ...
                        <div className="w-full lg:flex-1 space-y-8">
                            ...
                            <div className="bg-white dark:bg-[#050807] rounded-xl border border-gray-100 dark:border-white/5 p-8 shadow-sm">
                                <h3 className="text-sm font-black text-gray-900 dark:text-white mb-6 uppercase tracking-[2px] flex items-center gap-3">
                                    <div className="w-1 h-6 bg-[#00BA4A] rounded-full"></div> MÔ TẢ CHI TIẾT GIẢI PHÁP
                                </h3>
                                <div className="text-[15px] text-gray-600 dark:text-gray-400 leading-[1.8] whitespace-pre-wrap font-medium">
                                    {w.content || w.description}
                                    <br /><br />
                                    <ul className="space-y-3">
                                        <li className="flex items-start gap-3">
                                            <FeatherIcon icon="check-circle" size={16} className="text-[#00BA4A] mt-1 shrink-0" />
                            ...
                            {tags.length > 0 && (
                                <div className="bg-white dark:bg-[#050807] rounded-xl border border-gray-100 dark:border-white/5 p-8 shadow-sm">
                                    <h3 className="text-sm font-black text-gray-900 dark:text-white mb-6 uppercase tracking-[2px] flex items-center gap-3">
                                        <div className="w-1 h-6 bg-[#00BA4A] rounded-full"></div> PHẠM VI ỨNG DỤNG
                                    </h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        {tags.map((tag, i) => (
                                            <div key={i} className="flex items-center gap-4 group">
                                                <div className="w-8 h-8 rounded-lg bg-[#00BA4A]/10 flex items-center justify-center text-[#00BA4A] group-hover:bg-[#00BA4A] group-hover:text-white transition-all shrink-0">
                                                    <FeatherIcon icon="check" size={12} strokeWidth={4} />
                                                </div>
                                                <span className="text-[13px] font-bold text-gray-700 dark:text-gray-300 uppercase tracking-widest">{tag}</span>
                                            </div>
                                        ))}
                                    </div>
                                </div>
                        ...
                        <div className="w-full lg:w-[360px] shrink-0 space-y-6 lg:sticky lg:top-24">
                            <div className="bg-white dark:bg-[#0d1412] rounded-xl border border-gray-100 dark:border-white/5 p-8 shadow-[0_20px_50px_-12px_rgba(0,0,0,0.1)] space-y-8">
                                <div className="space-y-1">
                                    <div className="text-[10px] font-black text-gray-400 uppercase tracking-[2px]">Mức đầu tư giải pháp</div>
                                    <div className="text-4xl font-black text-[#00a340] tracking-tighter leading-none">{fmt(w.price)}</div>
                                ...
                                <div className="space-y-4">
                                    <button
                                        onClick={handleBuy}
                                        disabled={buying}
                                        className="w-full h-[52px] rounded-xl bg-gradient-to-r from-[#00BA4A] to-[#01a643] hover:brightness-105 !text-white text-[15px] font-black uppercase tracking-widest flex items-center justify-center gap-2 transition-all shadow-lg shadow-[#00BA4A]/20 active:scale-[0.98] disabled:opacity-50"
                                    >
                                        {buying ? (
                                            <>
                                ...
                                <div className="space-y-4 pt-8 border-t border-gray-50 dark:border-white/5">
                                    <div className="flex items-center gap-3 text-[13px] font-bold text-gray-600 dark:text-gray-400">
                                        <div className="w-6 h-6 rounded-full bg-[#00BA4A]/10 flex items-center justify-center text-[#00BA4A]">
                                            <FeatherIcon icon="check" size={12} strokeWidth={4} />
                                        </div>
